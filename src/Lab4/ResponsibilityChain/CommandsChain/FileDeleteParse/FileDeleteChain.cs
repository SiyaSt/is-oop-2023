...
public class FileDeleteChain : CommandChain
{
    private readonly IFileDeleteArgumentChain _parseArgument;
    private readonly IFileDeleteArgumentChain _parseFlag;
    public FileDeleteChain(IFileDeleteArgumentChain fileDeleteArgumentChain, IFileDeleteArgumentChain parseFlag)
    {
        _parseArgument = fileDeleteArgumentChain;
        _parseFlag = parseFlag;
    }

    public override ParseResultTypes Handle(Request request)
    {
        var resultModelBuilder = new FileDeleteModel.Builder();
        if (request.Command.Current != "delete")
        {
        ...
        return new ParseResultTypes.SuccessCommand(new FileDelete(resultModelBuilder.Build()));
    }
}